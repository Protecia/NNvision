        ...
        while self.running_rtsp :
            if i==15:
                ...
                if ret and len(frame)>100 :
                    ...
                    cs.logger.debug("resultat de l'ecriture du fichier jpg : {} en {} ".format(
    ...
    def run(self):
        ...
        while self.running :
                    ...
                    if r.status_code == 200 and len(r.content)>1000 :
                        with open(self.img_temp, 'wb') as fd:
                            #for chunk in r.iter_content(chunk_size=128):
                             fd.write(r.content)
                        cs.logger.info('image saved to temp folder for darknet in {}s '.format(
                                     time.time()-t))
                    else:
                        self.request_OK = False
            ...
            if self.request_OK:
                ...
                result_darknet = cs.detect(cs.net, cs.meta, self.img_temp.encode(),
                                               thresh=th,
                                               hier_thresh = 0.4)
                ...
                if self.base_condition(result_filtered) and cs.Camera.objects.get(id=
                                      self.cam.id).rec:
